...
describe("Property-Based Tests for Validators", () => {
  ...
  describe("Property 7: Focus Quality Range Validation", () => {
    it("should accept only integers 1-5 for focusQuality", () => {
      fc.assert(
        fc.property(fc.integer({ min: 1, max: 5 }), (focusQuality) => {
          const switchLogData = {
            focusQuality,
          };
      ...
      fc.assert(
        fc.property(
          ...
          (focusQuality) => {
            ...
            const result =
              updateSwitchLogPatchValidator.safeParse(switchLogData);

            // The validator should reject values outside 1-5
            expect(result.success).toBe(false);
            if (!result.success) {
              const focusErrors = result.error.issues.filter((issue) =>
                issue.path.includes("focusQuality"),
              );
