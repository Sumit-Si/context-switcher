...
describe('ContextService Unit Tests', () => {
    ...
    describe('checkDuplicate', () => {
        it('should return false when no duplicate exists', async () => {
            const isDuplicate = await contextService.checkDuplicate('Work', testUserId);
    ...
    describe('create', () => {
        ...
        it('should throw 409 error when duplicate name exists', async () => {
            ...
            await expect(contextService.create(contextData, testUserId)).rejects.toThrow(ApiError);
            await expect(contextService.create(contextData, testUserId)).rejects.toMatchObject({
                statusCode: 409,
    ...
    describe('getById', () => {
        it('should return context by ID for user', async () => {
            ...
            const context = await contextService.getById((created as any)._id.toString(), testUserId);

            expect(context.name).toBe('Work');
            expect((context as any)._id.toString()).toBe((created as any)._id.toString());
        });

        it('should throw 404 when context not found', async () => {
            const fakeId = new Types.ObjectId().toString();

            await expect(contextService.getById(fakeId, testUserId)).rejects.toThrow(ApiError);
            await expect(contextService.getById(fakeId, testUserId)).rejects.toMatchObject({
                statusCode: 404,
    ...
    describe('update', () => {
        ...
        it('should throw 409 when updating to duplicate name', async () => {
            ...
            await expect(
                contextService.update(toUpdate._id.toString(), { name: 'Existing' }, testUserId)
            ).rejects.toThrow(ApiError);
            await expect(
                contextService.update(toUpdate._id.toString(), { name: 'Existing' }, testUserId)
    ...
    describe('delete', () => {
        ...
        it('should throw 404 when context not found', async () => {
            ...
            await expect(contextService.delete(fakeId, testUserId)).rejects.toThrow(ApiError);
            await expect(contextService.delete(fakeId, testUserId)).rejects.toMatchObject({
                statusCode: 404,
            });
        });
