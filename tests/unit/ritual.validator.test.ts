  ...
  describe("createRitualPostValidator", () => {
    ...
    describe("description validation", () => {
      it("should accept optional description", () => {
        const { description: _description, ...dataWithoutDescription } =
          validRitualData;
        const result = createRitualPostValidator.safeParse(
      ...
      it("should reject description longer than 1000 characters", () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
          const descError = result.error.issues.find((e) =>
    ...
    describe("totalDuration validation", () => {
      ...
      it("should coerce string duration to number", () => {
        const dataWithStringDuration = {
          ...validRitualData,
          totalDuration: "300" as any,
        };
        const result = createRitualPostValidator.safeParse(
          dataWithStringDuration,
        );
    ...
    describe("steps validation", () => {
      it("should reject empty steps array", () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
          const stepsError = result.error.issues.find((e) =>
            e.path.includes("steps"),
          );
