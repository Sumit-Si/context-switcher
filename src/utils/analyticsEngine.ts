...
// ─────────────────────────────────────────────
export const computeAnalytics = async ({ userId, timeRange }: AnalyticsRequestParams) => {
    ...
    const focusScoreChange = previousMetrics.averageFocusScore
        ? Math.round(keyMetrics.averageFocusScore - previousMetrics.averageFocusScore)
        : 0;
...
const getKeyMetrics = async ({ userId, from, to }: DateRangeParams) => {
    ...
    return {
        ...
        ritualComplianceRate: r.switchesTotal > 0
...
const getContextSwitches = async ({ userId, from, to }: DateRangeParams) => {
    ...
    return switches.map((sw) => {
        ...
        const difficulty: Difficulty = focus < 50 ? "hard" : focus < 70 ? "medium" : "easy";
        return {
...
const getTimeOfDayData = async ({ userId, from, to }: DateRangeParams) => {
    ...
    results.forEach((r) => { hourMap[r._id] = Math.round(r.avgFocus); });
...
// ─────────────────────────────────────────────
const generateInsights = ({ keyMetrics, contextPerformance, contextSwitches, focusScoreChange }: InsightInput) => {
