...
const updateSwitchLogById = asyncHandler(async (req, res) => {
  ...
  const { focusQuality, distraction, notes, projectTag } =
    req.body as UpdateSwitchLogRequestBody;

  const updated = await switchLogService.update(
    id,
    {
      focusQuality,
      distraction,
...
const getActiveSession = asyncHandler(async (req, res) => {
  ...
  const switchLog = await switchLogService.getActiveSession(
    user._id.toString(),
  );
