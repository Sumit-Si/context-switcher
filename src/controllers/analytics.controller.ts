...
const getAnalytics = asyncHandler(async (req, res) => {
    try {
        ...
        if (!validRanges.includes(timeRange)) {
            throw new ApiError({ statusCode: 400, message: "Invalid timeRange. Use: day, week, month, all" });
        }
        ...
        return res
            .status(200)
            .json(new ApiResponse({ statusCode: 200, data, message: "Analytics computed successfully" }));
